...
export function MoodTrackerModal({ isOpen, onClose }: MoodTrackerModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(val) => !val && onClose()}>
      <DialogContent className="max-w-3xl h-[85vh] flex flex-col p-8 overflow-hidden rounded-[2.5rem] border-none shadow-2xl">
        <DialogHeader className="mb-6 flex flex-row items-center justify-between">
          <div>
            <DialogTitle className="text-3xl font-black tracking-tight">Mindful Check-in</DialogTitle>
            <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-1">Track your wellbeing & performance</p>
          </div>
          <div className="flex bg-gray-100 p-1 rounded-2xl">
            <button onClick={() => setView('log')} className={cn("px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all", view === 'log' ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600")}>Log</button>
            <button onClick={() => setView('analytics')} className={cn("px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all", view === 'analytics' ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600")}>Charts</button>
          </div>
        </DialogHeader>
        ...
        {view === 'log' ? (
          <div className="flex-1 overflow-y-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pr-2 no-scrollbar">
            <section>
              <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-6 flex items-center gap-2"><Smile className="w-4 h-4 text-blue-500" /> How are you feeling right now?</h3>
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                {MOODS.map(m => (
                  ...
                  >
                    ...
                    <span className={cn("text-[10px] font-black uppercase tracking-widest", selectedMood === m.id ? "text-gray-900" : "text-gray-400")}>{m.label}</span>
                  </button>
                ))}
              </div>
            </section>
            ...
            <section>
              <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-6 flex items-center gap-2"><Zap className="w-4 h-4 text-yellow-500" /> Energy Level: {energyLevel}/10</h3>
              <div className="px-4">
                <input 
                  type="range"
                  min="1"
            ...
            <section>
              <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-4 flex items-center gap-2"><MessageSquare className="w-4 h-4 text-purple-500" /> Notes / Journal</h3>
              <Textarea 
                placeholder="What's causing these feelings? Any wins today?" 
                value={note}
                onChange={e => setNote(e.target.value)}
                className="rounded-[2rem] h-32 bg-gray-50 border-none focus:ring-2 focus:ring-blue-600/10 transition-all p-6 text-lg resize-none"
              />
            </section>
            ...
            <section className="bg-blue-50/50 p-6 rounded-[2rem] border border-blue-100/50 flex items-start gap-4">
               <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center text-blue-600 shrink-0"><Info className="w-5 h-5"/></div>
               <div>
                  <h4 className="font-black text-blue-900 uppercase tracking-widest text-xs mb-2">Personal Tip</h4>
                  <ul className="space-y-1">
                    {moodSuggestions.map((s, i) => (
                      <li key={i} className="text-sm font-bold text-blue-700/70 flex items-center gap-2">
          ...
          <div className="flex-1 overflow-y-auto space-y-10 animate-in fade-in slide-in-from-right-4 duration-500 pr-2 scrollbar-hide">
             <section className="space-y-6">
                <div className="flex items-center justify-between">
                   <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest flex items-center gap-2"><TrendingUp className="w-4 h-4 text-green-500" /> Weekly Wellbeing</h3>
                ...
                <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                      ...
                      <YAxis stroke="#9ca3af" fontSize={10} tickLine={false} axisLine={false} width={20} domain={[0, 10]} hide />
                      <Tooltip 
                        contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', padding: '16px' }}
             ...
             <section>
               <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-6 flex items-center gap-2"><History className="w-4 h-4 text-gray-400" /> Recent reflections</h3>
               <div className="space-y-4">
                 {moods.slice(0, 5).map(m => {
                   const config = MOODS.find(mood => mood.id === m.mood);
                   return (
                     <div key={m.id} className="p-5 bg-gray-50 border border-gray-100 rounded-3xl flex gap-5">
                       <div className="text-3xl shrink-0 flex items-center justify-center w-14 h-14 bg-white rounded-2xl shadow-sm">{config?.emoji}</div>
                       <div className="flex-1 min-w-0">
                         <div className="flex items-center justify-between mb-1">
                            <h4 className="font-black text-gray-900 text-lg uppercase tracking-tight">{config?.label} <span className="text-gray-300 font-bold ml-2">/ Energy {m.energyLevel}</span></h4>
                            <span className="text-[10px] font-black text-gray-400 uppercase">{format(m.createdAt, 'MMM d, h:mm a')}</span>
                         </div>
                         <p className="text-sm text-gray-500 font-bold leading-relaxed line-clamp-2 italic">"{m.note || 'No notes added'}"</p>
                       </div>
                     </div>
                   );
                 })}
                 {moods.length === 0 && <p className="text-center py-10 text-gray-400 font-bold uppercase tracking-widest text-xs">No moods logged yet</p>}
               </div>
             </section>
          </div>
        )}
